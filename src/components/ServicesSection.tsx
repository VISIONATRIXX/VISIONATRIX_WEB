...
function CanvasSimulator({ type, mousePos, isHovered }: CanvasSimulatorProps) {
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      // n8n Multi-Branch Node Workflow Execution Flow Graph
      // -----------------------------------------------------------------
      } else if (type === "ai") {
        ...
        const totalDuration = 7.0; // 7 seconds per full cycle
        const progress = (time % totalDuration) / totalDuration;
        const currentStep = Math.floor(progress * 6); // 6 execution steps
      ...
      // Studio Stage Table, Softbox Lights, Shutter Lock & Flashing Strobe Click
      // -----------------------------------------------------------------
      } else if (type === "brand") {
        const cx = width / 2;
        const cy = height / 2;
        ...
        const flashCycle = (time * 1.3) % 2.8; // Flash triggers every 2.8s
